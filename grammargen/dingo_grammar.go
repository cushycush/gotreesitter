...
// DingoGrammar extends Go with Rust-inspired syntax sugar.
...
func DingoGrammar() *Grammar {
	return ExtendGrammar("dingo", GoGrammar(), func(g *Grammar) {

		// Mark dingo keywords as non-keyword strings so the keyword DFA
		...
		g.Define("_dingo_null_coalesce_op", Token(Seq(Str("?"), Str("?"))))

		g.Define("null_coalesce", PrecLeft(2,
			Seq(
				Field("left", Sym("_expression")),
				Sym("_dingo_null_coalesce_op"),
