...
var (
	dingoLangOnce   sync.Once
	dingoLangCached *gotreesitter.Language
	dingoLangErr    error
)

func getDingoLanguage() (*gotreesitter.Language, error) {
	dingoLangOnce.Do(func() {
		dingoLangCached, dingoLangErr = GenerateLanguage(DingoGrammar())
	})
	return dingoLangCached, dingoLangErr
}

// TranspileDingo converts .dingo source to valid Go code.
func TranspileDingo(source []byte) (string, error) {
	lang, err := getDingoLanguage()
	if err != nil {
		return "", fmt.Errorf("generate dingo language: %w", err)
	}
	...
	root := tree.RootNode()
	if root.HasError() {
		return "", fmt.Errorf("parse errors in dingo source")
	}

	t := &dingoTranspiler{src: source, lang: lang}
	return t.emit(root), nil
}

type dingoTranspiler struct {
	src  []byte
	lang *gotreesitter.Language
}

func (t *dingoTranspiler) text(n *gotreesitter.Node) string {
	return string(t.src[n.StartByte():n.EndByte()])
}

func (t *dingoTranspiler) nodeType(n *gotreesitter.Node) string {
	return n.Type(t.lang)
}

func (t *dingoTranspiler) childByField(n *gotreesitter.Node, field string) *gotreesitter.Node {
	return n.ChildByFieldName(field, t.lang)
}

func (t *dingoTranspiler) emit(n *gotreesitter.Node) string {
	switch t.nodeType(n) {
...
func (t *dingoTranspiler) emitDefault(n *gotreesitter.Node) string {
	cc := int(n.ChildCount())
	if cc == 0 {
		return t.text(n)
	}
...
func (t *dingoTranspiler) emitEnum(n *gotreesitter.Node) string {
	name := "Enum"
...
// let x = 1 -> x := 1
func (t *dingoTranspiler) emitLet(n *gotreesitter.Node) string {
...
func (t *dingoTranspiler) emitMatch(n *gotreesitter.Node) string {
...
func (t *dingoTranspiler) emitNullCoalesce(n *gotreesitter.Node) string {
...
func (t *dingoTranspiler) emitErrorProp(n *gotreesitter.Node) string {
...
func (t *dingoTranspiler) emitSafeNav(n *gotreesitter.Node) string {
...
func (t *dingoTranspiler) emitLambda(n *gotreesitter.Node) string {
