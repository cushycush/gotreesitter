...
func Normalize(g *Grammar) (*NormalizedGrammar, error) {
	...
	for _, name := range nonterminals {
		...
		// When a rule's entire body is repeat/repeat1, expand to a
...
// expandTopLevelRepeat expands repeat/repeat1 at the top level of a rule
// into a self-referencing choice. This matches tree-sitter's behavior:
// when a rule IS a repeat, tree-sitter makes the rule self-recursive
...
func expandTopLevelRepeat(r *Rule, ruleName string) *Rule {
	...
	inner := r
	var precWrappers []*Rule // stack of wrappers to re-apply
